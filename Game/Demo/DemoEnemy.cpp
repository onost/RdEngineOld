	...
	, mSpeed(2.5f)
{
	...
	bc->SetAttribute(CollisionAttribute::kEnemies);// 敵
	bc->SetResponse(CollisionAttribute::kAll);
...
void DemoEnemy::ActorUpdate(float deltaTime)
{
	...
	RaycastInfo info = {};
	if (mScene->GetCollisionManager()->Raycast(ray, info, CollisionAttribute::kTerrain))
	{
